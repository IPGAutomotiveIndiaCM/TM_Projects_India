...
extern const char SwitchMobility_ACC_LibIdent[];
const char SwitchMobility_ACC_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_SwitchMobility_ACC *rtm)
{
...
static void
SwitchMobility_ACC_DeclQuants (void *MP)
{
    rtModel_SwitchMobility_ACC *rtm = (rtModel_SwitchMobility_ACC *)MP;

    /*Log("%s_DeclQuants()\n", Modelname);*/

    if (rtm == NULL) {
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(SwitchMobility_ACC_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	MatSupp_DeclQuants(SwitchMobility_ACC_DictDefines);
...
/*
 * SwitchMobility_ACC_SetParams() will be invoked indirectly by the generated
 * model C code each time SwitchMobility_ACC_New() is called.
 */
void
SwitchMobility_ACC_SetParams (rtModel_SwitchMobility_ACC *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
static void *
SwitchMobility_ACC_New (struct tInfos *Inf, const char *KindKey)
{
    rtModel_SwitchMobility_ACC *rtm;
    ...
    MatSupp_ResetQuants(SwitchMobility_ACC_DictDefines);
...
static void
SwitchMobility_ACC_Delete (void *MP)
{
    rtModel_SwitchMobility_ACC *rtm = (rtModel_SwitchMobility_ACC *)MP;
...
static int
SwitchMobility_ACC_Calc (void *MP, double dt)
{
    rtModel_SwitchMobility_ACC *rtm = (rtModel_SwitchMobility_ACC *)MP;
    int osCount=0;
    ExternalInputs_SwitchMobility_ACC  *in  = (ExternalInputs_SwitchMobility_ACC *) rtmGetU(rtm);
    ExternalOutputs_SwitchMobility_ACC *out = (ExternalOutputs_SwitchMobility_ACC *)rtmGetY(rtm);
    ...
    in->FromCM.DigitalSelectorCtrl.DGS0 = VehicleControl.DigitalSelectorCtrl[0];
    in->FromCM.DigitalSelectorCtrl.DGS1 = VehicleControl.DigitalSelectorCtrl[1];
    in->FromCM.DigitalSelectorCtrl.DGS2 = VehicleControl.DigitalSelectorCtrl[2];
...
int
SwitchMobility_ACC_Register (void)
{
    tModelClassDescr m;
    ...
    m.VehicleControl.DeclQuants		= SwitchMobility_ACC_DeclQuants;
    m.VehicleControl.New		= SwitchMobility_ACC_New;
    m.VehicleControl.Calc		= SwitchMobility_ACC_Calc;
    m.VehicleControl.Delete		= SwitchMobility_ACC_Delete;
